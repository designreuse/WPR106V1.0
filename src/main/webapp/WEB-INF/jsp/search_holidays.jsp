      		...
      		<tr>
        		<td valign="top" align="left">
			        <div class="headings altheading">
			          <h2>Holiday Information<br/></h2><div class="searchdiv">
...
<form action="findholidays" method="GET">
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0" >
							  ...
							  <tr>
							   ...
							   <c:forEach items="${holidaysForm.holidays}" var="holidays" varStatus="status">
							    <option value="${holidays.org_name}">${holidays.org_name}</option>
							    </c:forEach>
							    </select></td>
							    ...
							    <select   id="e2"style="width:260px;" name="branch">
							 	<option value="">Select None</option>     
							   <c:forEach items="${holidaysForm.holidays}" var="holidays" varStatus="status">
							    <option value="${holidays.branch}">${holidays.branch}</option>
							    </c:forEach>
							    </select></td>
	...
	<input type="text" id="search" name="search" placeholder="Enter Text To Filter" class="light-table-filter" data-table="order-table" placeholder="Filter"/>
				        ...
				        <table cellpadding="0" cellspacing="0" border="0" width="100%" style="width:100%">
							<c:if test="${fn:length(holidaysForm1.holidays) gt 0 }">
        					<c:forEach items="${holidaysForm1.holidays}" var="holiday" varStatus="status">
        				       					<tr class="row1">
										...
										<a href="<c:out value="deleteholidays?org_name=${holiday.org_name}&branch=${holiday.branch}&holiday_date=${holiday.holiday_date}"/>" onclick="return confirmation('Are you sure want to Delete?')"><img src="resources/images/del.png" alt="Delete" width="20"height="18" title="Delete"/></a>
									
											</td>
								</tr>
							    	</c:forEach>
							    </c:if>
							    <c:if test="${fn:length(holidaysForm1.holidays) == 0}">	
							    	<tr class="row1">
